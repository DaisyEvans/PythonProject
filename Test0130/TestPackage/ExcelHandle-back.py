...
# ps：如同存，若活期待偿期为空，计算加权平均待偿期时，会算上活期的规模，拉低平均待偿期


def main():
    filename = r"C:\Users\LHF\Desktop\指定成本与FIFO损益分析\20190417170316.xls"
    ...
    for sma_rowindex in range(len(sma_rowindex_list)):
        ...
        for pointer in range(pointer_begin, pointer_end):
            # 债券不考虑负值（第一大类是债券）
            if table.cell_type(pointer, maketValueCol) == 2 and (
                    table.cell_value(pointer, maketValueCol) > 0 or lar_rowindex > 0):
                # 盯市：市值+应计利息；市场净价收益率
                if caliber == 0:
                    # 剔除非数值型数据
                ...
                # 折溢摊：折溢摊成本+应计利息；折溢摊净价收益率
                    ...
                    if table.cell_type(pointer, amortCostCol) == 2 and table.cell_type(pointer,
                                                                                       accruedInterestCol) == 2:
